...
extension OpenAIAPI {
    public struct CompletionRequest: Codable {
        ...
        public init(prompt: String, model: String = "text-davinci-003", max_tokens: Int = 1500, temperature: Double = 0.2, stop: [String]? = nil, user: String? = nil) {
            self.prompt = prompt
            self.model = model
    ...
    private func createTextRequest(completionRequest: CompletionRequest) throws -> URLRequest {
        let url = URL(string: textCompletionEndPoint)!
